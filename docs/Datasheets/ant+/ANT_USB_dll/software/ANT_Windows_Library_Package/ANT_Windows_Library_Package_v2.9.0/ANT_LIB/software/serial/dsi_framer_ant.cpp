...
USHORT DSIFramerANT::WaitForMessage(ULONG ulMilliseconds_)
{
   ...
   if ((usMessageSize == DSI_FRAMER_TIMEDOUT) && (ulMilliseconds_ != 0))
   {
      ...
      {
        ucError = (UCHAR)DSI_FRAMER_ERROR; //Set ucError so we can distinguish from a normal error if this ever occurs
...
///Allocates a new standard ANT_MESSAGE struct which must be free() after use. 
...
{         
   ...
   {
      *ppstExtBufAntMsg_ = NULL;
      return FALSE;
   }
   else
   {
      *ppstExtBufAntMsg_ = (ANT_MESSAGE*)malloc(sizeof(ANT_MESSAGE));
      return TRUE;
   }
}
