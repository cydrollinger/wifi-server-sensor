...
void DSIANTDevice::HandleSerialError(void)
{
   ...
   CloseDevice(TRUE);
      ...
      // Reset the USB stick.
      ((DSISerialGeneric*)pclSerialObject)->USBReset();
   #endif   
      ...
      if (usMesgSize < DSI_FRAMER_TIMEDOUT)  //if the return isn't DSI_FRAMER_TIMEDOUT or DSI_FRAMER_ERROR
	   {
         ...
         if (usMesgSize == 0)
         {
            ...
            SNPRINTF((char *)aucString2,256, "Rx:%u[0x%02X]...[0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X]", usMesgSize , stMessage.ucMessageID, stMessage.aucData[0], stMessage.aucData[1], stMessage.aucData[2], stMessage.aucData[3], stMessage.aucData[4], stMessage.aucData[5], stMessage.aucData[6], stMessage.aucData[7]);
