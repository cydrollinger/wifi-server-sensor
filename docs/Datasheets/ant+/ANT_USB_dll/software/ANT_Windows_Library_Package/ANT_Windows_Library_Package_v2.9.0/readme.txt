...
Project Hierarchy ...
             ...
             |                  |                        |
             |                  |                        |
          ANT_DLL (C++)    DEMO_LIB (C++)      ANT_Unmanaged_Wrapper (C++)
             |                                           |
             |                                           |
             |                                           |
...
The projects in this directory have been designed and tested using the free
version of the Visual C++ and C# Compilers (version 2008) from Microsoft available here:
...
The Visual studio solutions included in this package have been designed to handle all
the dependencies between projects.  Make sure to open the solution files instead of 
individual project files. In order to handle dependencies correctly, please build
ANT_Libraries.sln first, and then build ANT_NET_Libraries.sln
...
*** Visual Studio Solution: ANT_NET_Libraries.sln (C#)
...
the file ANT_LIB\software\ANTFS\antfsmessage.h, set the key to the 
ANT-FS network key or to all 0's.  You will get an error from the compiler 
prompting you to set the network key unless it has been set.

Similarly, before the DEMO_ANTFS will compile, the ANT-FS network key must be set. Inside 
the file DEMO_ANTFS\demo_host.cs, set the key to the ANT-FS network
key or to all 0's to use the public key.

---------------------------------------------------------------------------------
** Important ** Silabs Dynamic Libraries
...
---------------------------------------------------------------------------------
** Important ** Managed Applications for Windows 64-bit
